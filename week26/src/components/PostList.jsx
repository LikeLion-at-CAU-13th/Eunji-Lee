...
function PostList({ variant = FEED_VARIANT.HOME_FEED, showPostForm }) {
  ...
  if (variant === FEED_VARIANT.HOME_FEED) {
    postsQueryKey = ["posts", FEED_VARIANT.HOME_FEED];
    postsQueryFn = getPosts; // 전체 포스트 조회 함수
  } else if (variant === FEED_VARIANT.MY_FEED) {
    postsQueryKey = ["posts", FEED_VARIANT.MY_FEED];
    postsQueryFn = getPostsByUsername; // 내 포스트 조회 함수
  }
